"""
Train the OC (Out-of-Context) branch: PhoBERT-base + ViT, comment-only text.
...
Usage:
    python train_oc.py --data-dir . --output oc_model.pth
"""
import argparse
import os
import random
...
class CommentOnlyDataset(Dataset):
    """OC branch: uses only the comment text + thumbnail image."""
...
def main():
    ...
    parser.add_argument("--output", default="oc_model.pth")
    ...
    def make_loader(df, shuffle):
        ds = CommentOnlyDataset(df, image_dir, tokenizer, args.max_length)
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        running_loss = 0.0
        for batch in tqdm(train_loader, desc=f"[OC] epoch {epoch}"):
            input_ids = batch["input_ids"].to(device)
            attention_mask = batch["attention_mask"].to(device)
            image = batch["image"].to(device)
            labels = batch["label"].to(device)
    ...
    print(f"OC TEST metrics: {test_metrics}")

    torch.save(best_state, args.output)
    print(f"Saved best OC checkpoint to {args.output}")
